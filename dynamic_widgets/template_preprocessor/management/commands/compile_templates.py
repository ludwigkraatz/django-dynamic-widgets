...
from dynamic_widgets.template_preprocessor.utils import language, template_iterator, load_template_source, get_template_path
from dynamic_widgets.template_preprocessor.utils import get_options_for_path, execute_precompile_command
from dynamic_widgets.template_preprocessor.core.utils import need_to_be_recompiled, create_media_output_path
...
class Command(BaseCommand):
    ...
    def _build_compile_queue(self, languages, all_templates=True, single_template=None):
        ...
        for lang in languages:
            ...
            if all_templates and not single_template:
                ...
                input_path = single_template
                output_path = self._make_output_path(lang, single_template)
                queue.add( (lang, single_template, input_path, output_path) )
    ...
    def _build_compile_media_queue(self, languages):
        from dynamic_widgets.template_preprocessor.core.utils import compile_external_css_files, compile_external_javascript_files
    ...
    def _compile_template(self, lang, template, input_path, output_path, no_html=False):
        try:
            ...
            #
            from dynamic_widgets.template_loader import WidgetLoader
            code, origin = WidgetLoader().load_template_source(template)
            """try:
                # Open input file
            ...
            if no_html:
                ...
                output, context = compile(code, path=input_path, loader=lambda path: code,
