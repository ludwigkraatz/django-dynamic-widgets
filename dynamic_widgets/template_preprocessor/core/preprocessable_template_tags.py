...
from dynamic_widgets.template_preprocessor import preproces_tag
...
def preprocess_tag(func_or_name, takes_context=False):
    """
    > @preprocess_tag
    ...
    if isinstance(func_or_name, basestring):
        def decorator(func):
            __preprocessabel_tags[func_or_name] = (func, takes_context)
            return func
        return decorator
    else:
        __preprocessabel_tags[func_or_name.__name__] = (func_or_name, takes_context)
        return func_or_name
