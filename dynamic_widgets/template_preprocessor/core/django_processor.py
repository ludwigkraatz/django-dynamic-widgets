...
from dynamic_widgets.template_preprocessor.core.lexer import Token, State, StartToken, Shift, StopToken, Push, Pop, Error, Record, CompileException
from dynamic_widgets.template_preprocessor.core.preprocessable_template_tags import get_preprocessable_tags, NotPreprocessable
from dynamic_widgets.template_preprocessor.core.lexer_engine import nest_block_level_elements, tokenize
import re
...
class DynamicContentTag(Token):
    """
    {% dynamic_content varname_or_template %}

    Support for with-parameters:
    {% include "name_snippet.html" with person="Jane" greeting="Hello" if profile.has_picture %}
    """
    def process_params(self, params):
        include_param = params[1].output_as_string()
        ...
        while index < len(params):
            # With parameters
            if params[index].output_as_string() == 'with':
                if not 'if' in params[index+1:]:
                    self.params += ('with', params[index+1:])
                    index = len(params)
                else:
                    condition_index = params[index+1:].index('if')
                    self.params += ('with', params[index+1:condition_index+index+1])
                    index = condition_index
    
            # Condition parameters
            elif params[index].output_as_string() == 'if':
                if not 'with' in params[index+1:]:
                    self.params += ('if', params[index+1:])
                    index = len(params)
                else:
                    with_index = params[index+1:].index('with')
                    self.params += ('if', params[index:with_index+index+1])
                    index = with_index
    ...
    def output(self, handler):
        ...
        if self.template_name_is_variable:
            handler(u'{%dynamic_content '); handler(self.template_name); handler(u'%}')
        else:
            handler(u'{%dynamic_content "'); handler(self.template_name); handler(u'"%}')
...
_DJANGO_INLINE_ELEMENTS = {
    ...
    'dynamic_content': DynamicContentTag,
    'url': DjangoUrlTag,
    'load': DjangoLoadTag,
...
class PreprocessedDynamicContent(DjangoContainer):
    def init(self, children, params=None):
        self.children = children
        self.params = params
...
def _preprocess_dynamic_content(tree, context):
    """
    Look for all the {% dynamic_content ... %} tags and replace it by their content.
    """
    include_blocks = list(tree.child_nodes_of_class(DynamicContentTag))
    ...
    for block in include_blocks:
        if not block.template_name_is_variable:
            try:
                ...
                # Parse include
                include_tree = parse(context.load_dynamic_content(block.template_name), '?', context, True)

                # Move tree from included file into {% include %}
                block.__class__ = PreprocessedDynamicContent
                block.init([ include_tree ], block.params)

            except TemplateDoesNotExist, e:
                raise CompileException(block, 'Dynamic Content in {%% dynamic_content %%} tag not found (%s)' % block.template_name)
...
def _preprocess_lookups(tree, values_dict):
    """
    Replace known variables, like {{ MEDIA_URL }} by their value.
    """
    ...
    for var in tree.child_nodes_of_class(DjangoVariable):
        if var.varname in values_dict:
            pass
        #else:
        #    raise Exception, values_dict.get_lookups()
    for tag in tree.child_nodes_of_class(DjangoTag):
        for var in (arg for arg in tag.args if not (arg.startswith('"') or arg.startswith("'"))):
            values_dict.__contains__(var)
    for tag in tree.child_nodes_of_class(DjangoIfTag):
        for var in (arg for arg in (p.output_as_string() for p in tag._params[1:]) if not (arg.startswith('"') or arg.startswith("'"))):
            values_dict.__contains__(var)
    for tag in tree.child_nodes_of_class(DjangoIfEqualTag):
        for var in (arg for arg in (p.output_as_string() for p in tag._params[1:]) if not (arg.startswith('"') or arg.startswith("'"))):
            values_dict.__contains__(var)
...
def _execute_preprocessable_tags(tree, storing_context):
    preprocessable_tags_bundle = get_preprocessable_tags()

    for c in tree.all_children:
        if isinstance(c, DjangoTag) and c.tagname in preprocessable_tags_bundle:
            params = [ p.output_as_string() for p in c.get_childnodes_with_name('django-tag-element') ]
            try:
                preprocessable_tag = preprocessable_tags[c.tagname][0]
                needs_context = preprocessable_tags[c.tagname][1]
                if needs_context:
                    c.children = [ preprocessable_tag(storing_context, *params) ]
                else:
                    c.children = [ preprocessable_tag(*params) ]
                c.__class__ = DjangoContent
            except NotPreprocessable:
                pass

        elif isinstance(c, DjangoContainer):
            _execute_preprocessable_tags(c, storing_context)
...
from dynamic_widgets.template_preprocessor.core.html_processor import compile_html
...
def parse(source_code, path, context, main_template=False):
    ...
    _preprocess_dynamic_content(tree, context)
    ...
    if main_template:
        ...
        # get variable lookup inprint into context
        _preprocess_lookups(tree, context.get_variable_lookup_wrapper())
        ...
        # Preprocessable tags
        if False and options.execute_preprocessable_tags: # TODO
            storing_context = context # TODO storing Context
            _execute_preprocessable_tags(tree, storing_context)
            
            # Do variable lookups
            if options.preprocess_variables:
                _preprocess_variables(tree,
                            storing_context.get_store())
